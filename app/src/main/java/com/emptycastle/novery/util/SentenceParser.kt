...
data class ParsedSentence(
    ...
    companion object {
        const val DEFAULT_PAUSE_MS = 350
        const val SHORT_PAUSE_MS = 200
        const val LONG_PAUSE_MS = 500
        const val ELLIPSIS_PAUSE_MS = 600
    }
}
...
object SentenceParser {
    ...
    private fun shouldBreakAtEllipsis(text: String, afterEllipsis: Int): Boolean {
        ...
        return when {
            ...
            // Opening quote = new sentence
            nextChar in OPENING_QUOTES -> true
    ...
    private fun normalizeSentence(text: String): String {
        ...
        // Normalize ellipsis for consistent TTS
        result = result.replace("...", "…")
    ...
    private fun calculatePause(sentence: String): Int {
        ...
        return when (effectiveLast) {
            '?' -> ParsedSentence.DEFAULT_PAUSE_MS + 100
            '!' -> ParsedSentence.DEFAULT_PAUSE_MS + 50
            '—', '–' -> ParsedSentence.SHORT_PAUSE_MS
            else -> ParsedSentence.DEFAULT_PAUSE_MS
        }
    }
